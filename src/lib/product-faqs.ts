...
const TRAIT_FAQS: Partial<Record<Trait, (p: Product) => Faq[]>> = {
  ...
  barrier: () => [
    ...
    { q: 'Can I have a full barrier and full recyclability?', a: 'Not currently, and it is better to be straight about it. Multi-layer laminates block best precisely because they combine materials, which is what makes them hard to recycle. Mono-material films are recyclable through store drop-off but give a shorter shelf life.' },
  ],
  gifting: () => [
...
function universal(p: Product, t: Set<Trait>): Faq[] {
  ...
  return [
    ...
    { q: 'Where do you deliver?', a: `All 50 US states, the United Kingdom, Canada and Australia, on the same timeline everywhere: 3 to 5 business days of production after artwork approval, then 2 to 7 business days in transit. Every run is made to order and shipped directly to your address rather than pulled from stock, so you receive current production rather than whatever has been sitting in a warehouse. Freight is costed separately by weight, volume and destination. ${r('delivery')}` },
